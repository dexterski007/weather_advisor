...
class TestRecommendRoute(unittest.TestCase):
    ...
    def test_recommend_activity_weather_error(self, mock_cache_set, mock_cache_get, mock_requests_get):
        '''Test that an error is returned when weather data cannot be fetched'''
        ...
        self.assertEqual(response.status_code, 400)
        self.assertEqual(response.json, {"error": "Unable to fetch weather data"})

    @patch('app.services.requests.get')
    @patch('app.services.suggest_activity')
    @patch('app.services.cache.get')
    @patch('app.services.cache.set')
    def test_recommend_activity_success(self, mock_cache_set, mock_cache_get, mock_suggest_activity, mock_requests_get):
        '''Test that an activity is returned when weather data is successfully fetched'''
        mock_cache_get.return_value = None
        mock_response = Mock()
